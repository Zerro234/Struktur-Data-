...
void TaskManager::tambahTugas() {
    ...
    do {
        ...
        cout << "(Ketik semua angka untuk lanjut kecuali 0 untuk kembali ke menu utama): ";
    	int index;
    	cin >> index;
    	if (index == 0) return;
...
void TaskManager::tampilkanTugas() const {
    system("cls");
    cout << "========================" << endl;
    cout << "     Tampilkan Tugas    " << endl;
    cout << "========================" << endl;

    cout << "-> [Tugas Belum Selesai] ->" << endl;
    for (size_t i = 0; i < daftarTugas.size(); ++i) {
        if (!daftarTugas[i].isDone) {
            cout << "\n"<< i + 1 << ". " << daftarTugas[i].nama << "\n	Keterangan : " << daftarTugas[i].keterangan << "\n	Deadlibne  : " << daftarTugas[i].deadline << "\n"<< endl;
        }
    }
    ...
    for (size_t i = 0; i < daftarTugas.size(); ++i) {
        if (daftarTugas[i].isDone) {
            cout << "\n"<< i + 1 << ". " << daftarTugas[i].nama << "\n	Keterangan	: " << daftarTugas[i].keterangan << "\n	Deadline	: " << daftarTugas[i].deadline << "\n 	Selesai pada	: " << daftarTugas[i].selesai << "\n" << endl;
        }
    }
	system("pause");
}
...
void TaskManager::tampilkanTugasSudahSelesai() const {
	cout << "Tugas Sudah Selesai di Kerjakan" << endl;
    for (size_t i = 0; i < daftarTugas.size(); ++i) {
        if (daftarTugas[i].isDone) {
...
void TaskManager::capTugasTuntas() {
    ...
    cout << "\nInput nomor tugas yang ingin dinyatakan selesai: ";
    int index;
    cin >> index;
...
void TaskManager::hapusTugas() {
    ...
    tampilkanTugasSudahSelesai(), cout <<" " <<endl;
    tampilkanTugasBelumSelesai();
...
void TaskManager::tambahJadwal() {
    ...
    do {
        ...
        cout << "Input Hari & Waktu makul (contoh Senin -> 07:30 - 09:20): ";
        cin >> ws;
        getline(cin, waktu);
...
void TaskManager::tampilkanJadwal() const {
    ...
    if (jadwal.empty()) {
        ...
        for (size_t i = 0; i < jadwal.size(); ++i) {
            cout << i + 1 << ". " << jadwal[i].namaMakul << " - [ " << jadwal[i].namaDosen << " ] - | " << jadwal[i].sks << " SKS | - " << jadwal[i].waktu <<"\n"<< endl;
        }
    }
    system("pause");
}

void TaskManager::tampilkanJadwalIni() const {
    if (jadwal.empty()) {
        cout << "Jadwal kosong." << endl;
    } else {
        for (size_t i = 0; i < jadwal.size(); ++i) {
...
void TaskManager::hapusJadwal() {
    ...
    tampilkanJadwalIni();
	cout << "\nInput nomor jadwal yang ingin dihapus (ketik -1 untuk reset semua, 0 untuk kembali ke menu utama): ";
    if (jadwal.empty()) {
        cout << "Jadwal kosong." << endl;
        return;
    }
...
void TaskManager::tampilkanNilaiIni() const {
    if (nilai.empty()) {
        cout << "Nilai kosong." << endl;
    } else {
        for (const auto& g : nilai) {
